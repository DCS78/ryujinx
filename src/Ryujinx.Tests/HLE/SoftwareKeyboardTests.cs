using NUnit.Framework;
using Ryujinx.HLE.HOS.Applets;
using System.Text;

namespace Ryujinx.Tests.HLE
{
    public class SoftwareKeyboardTests
    {
        [Test]
        public void StripUnicodeControlCodes_NullInput()
        {
            Assert.That(SoftwareKeyboardApplet.StripUnicodeControlCodes(null), Is.Null);
        }

        [Test]
        public void StripUnicodeControlCodes_EmptyInput()
        {
            Assert.That(SoftwareKeyboardApplet.StripUnicodeControlCodes(string.Empty), Is.EqualTo(string.Empty));
        }

        [Test]
        public void StripUnicodeControlCodes_Passthrough()
        {
            string[] prompts =
            [
                "Please name him.",
                "Name her, too.",
                "Name your friend.",
                "Name another friend.",
                "Name your pet.",
                "Favorite homemade food?",
                "What’s your favorite thing?",
                "Are you sure?"
            ];

            foreach (string prompt in prompts)
            {
                Assert.That(SoftwareKeyboardApplet.StripUnicodeControlCodes(prompt), Is.EqualTo(prompt));
            }
        }

        [Test]
        public void StripUnicodeControlCodes_StripsNewlines()
        {
            Assert.That(SoftwareKeyboardApplet.StripUnicodeControlCodes("I \r\nam \r\nvery \r\ntall"), Is.EqualTo("I am very tall"));
        }

        [Test]
        public void StripUnicodeControlCodes_StripsDeviceControls()
        {
            // 0x13 is control code DC3 used by some games
            string specialInput = Encoding.UTF8.GetString(new byte[] { 0x13, 0x53, 0x68, 0x69, 0x6E, 0x65, 0x13 });
            Assert.That(SoftwareKeyboardApplet.StripUnicodeControlCodes(specialInput), Is.EqualTo("Shine"));
        }

        [Test]
        public void StripUnicodeControlCodes_StripsToEmptyString()
        {
            string specialInput = Encoding.UTF8.GetString(new byte[] { 17, 18, 19, 20 }); // DC1 - DC4 special codes
            Assert.That(SoftwareKeyboardApplet.StripUnicodeControlCodes(specialInput), Is.EqualTo(string.Empty));
        }

        [Test]
        public void StripUnicodeControlCodes_PreservesMultiCodePoints()
        {
            // Turtles are a good example of multi-codepoint Unicode chars
            string specialInput = "♀ 🐢 🐢 ♂ ";
            Assert.That(SoftwareKeyboardApplet.StripUnicodeControlCodes(specialInput), Is.EqualTo(specialInput));
        }
    }
}
