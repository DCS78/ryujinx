...
namespace Ryujinx.HLE
{
 ...
 // If there aren't any readings, the default should be100% if still being measured, or0% if not.
 ...
 lock (_frameLock[frameType])
 {
 // Accumulate guest elapsed time only once.
 _accumulatedFrameTime[frameType] += elapsedFrameTime;

 // Count rendered host frames. When doubling is enabled, increment by2.
 _framesRendered[frameType] += increment;
 }
 }
